...
<!-- 最大10户逾期金额 -->
<template>
  <div class="risk_overdue">
    <!-- 内容区 -->
    <div class="content" :style="chartBackground">
      <!-- 图表区域 -->
      <div class="chart">
        <div id="myChart_overdue" class="content_chart"></div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
                             var colorList = [
                                 '#F0E2DF','#F2CBD0','#F5B5B6','#F6A09F','#F78988',
                                 '#F87674','#FA5F5A','#FA4B46','#FF3535','#FF1F1D'
                             ];
      ...
                             position: 'right',
                             formatter:function(params){
                                 return '{a|' + params.value/100 + '%}' + "  " +  '{b|' + accounting.formatMoney(params.value/22.64, "") + '}';
                             },
                             rich: {
                                  a: {
                                      color: '#3E637E',
  ...
  methods: {
    ...
    draw() {
        // 初始化echarts实例
        let myChart = this.$echarts.init(document.getElementById('myChart_overdue'))
        // 绘制图表
        // 防止越界，重绘canvas
